...
def usage():
	...
	print("-a output snort rule from all packet")
...
def basicconvert(singlepacket, packetnr0):
	try:
		print("\n{1}# ----- Snort Rules For Packet Number {0}-----{2}".format(packetnr0 + 1, G, W))
...
#Let user input pcap
		...
		protocol = None
		snortoutput = False
		options, arguments = getopt.getopt(sys.argv[1:], "r:p:ash")
...
# Check if a packet number has been supplied, and thus the variable packetnr is filled
		...
		if packetnr != None:
			if packetall: packetnr = range(1,len(scapy_cap) + 1)
			for i in packetnr:
				packetnr0 = int(i) - 1
...
# Check if the -s paramater is give, and thus the script needs to output Snort.
				if snortoutput == True:
					...
					if not packetall:loop_convert(locals())
...
def loop_convert(all_vars):
	while 1:
		try:
			all_vars['packetnr'] = input('\n# Summary:' + str(all_vars['scapy_cap']) + '\n# Please enter packets number(split with ,):').split(',')
			if all_vars['packetnr'] != None:
				for i in all_vars['packetnr']:
					packetnr0 = int(i) - 1
					singlepacket = all_vars['scapy_cap'][int(packetnr0)]
					# Check if the -s paramater is give, and thus the script needs to output Snort.
					if all_vars['snortoutput'] == True:
						basicconvert(singlepacket, packetnr0)
					else:
						print(str(singlepacket.show()))
			all_vars['packetnr'] = None
		except KeyboardInterrupt as e:
			exit()
		except Exception as e:
			print(get_err_msg())

def get_err_msg(_interval = ' --> '):
	call_str = ''; lcs = ''; _frame = sys._getframe()
	while _frame:
		No = _frame.f_lineno
		...
		if call_str == '':call_str = '|'
			...
			if lcs == '':
				lcs = _frame.f_locals
				No = sys.exc_info()[2].tb_lineno
			call_str = '%s(%s)%s%s'%(cname, No, _interval, call_str)
		_frame = _frame.f_back
	return call_str[0:-len(_interval)] + ': %s(%s)\n%s\n%s'%(sys.exc_info()[1],sys.exc_info()[0],'-'*79,lcs)
